...
public class TransactionLogEntry {
    ...
    static byte[] getLogEntry(XidImpl xid, String file, long filePosition, int fileContentLength,
            byte appendOrUndoTruncate) {
        byte filePathBytes[] = file.getBytes(UTF8Charset);
    ...
    static byte[] getLogEntry(XidImpl xid, String file, long newLength, byte truncateOrUndoAppend) {
        byte filePathBytes[] = file.getBytes(UTF8Charset);
        int filePathLength = filePathBytes.length;
        ByteBuffer buffer = ByteBuffer.allocate(200 + filePathLength);
    ...
    static byte[] getLogEntry(XidImpl xid, String sourceFile, String destinationFile, byte moveOrCopyOrUndoDelete) {
        byte sourceFilePathBytes[] = sourceFile.getBytes(UTF8Charset);
        byte destFilePathBytes[] = destinationFile.getBytes(UTF8Charset);
    ...
    static byte[] getLogEntry(XidImpl xid, byte commitStatus) {
    ...
    static byte[] getLogEntry(XidImpl xid, Set<File> files) {
        byte filePathsBytes[][] = new byte[files.size()][];
        int totalFilePathsLength = 0;
        int i = 0;
        Iterator<File> iter = files.iterator();
        while (iter.hasNext()) {
            filePathsBytes[i] = iter.next().getAbsolutePath().getBytes(UTF8Charset);
    ...
    private static String readFileName(ByteBuffer buffer) {
        ...
        buffer.get(fileName);
        return new String(fileName, UTF8Charset);
    }

    private static byte[] getBytesFromEvent(FileStateChangeEvent event) {
        byte fileNameBytes[] = event.getFile().getAbsolutePath().getBytes(UTF8Charset);
    ...
    private static FileStateChangeEvent readEvent(ByteBuffer buffer, XidImpl enqueuingTransaction) {
        ...
        String fileName = new String(fileNameBytes, UTF8Charset);
    ...
    static void updateContentLength(ByteBuffer buffer, int contentLength) {
        buffer.putInt(4, contentLength);
    }

    static TransactionLogEntry getNextTransactionLogEntry(FileChannel logChannel, long position,
